...
def detect_fish(source, weights, view_img, save_txt, imgsz, trace, save_image, project, name, exist_ok,
                device, augment, conf_thres, iou_thres, classes, agnostic_nms, save_conf, select_fishes):
    ...
    for path, img, im0s, vid_cap in dataset: # path=['0'], img=[image], vid_cap=None
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det): # !!!!!!!
                ...
                print("---------------------------------------------------")
            else: # not detect/bare conveyor
                if len(check_type_fish) > 0:
                    no_change += 1
                    print(no_change)
                    if no_change >= 2:
                        end_time  = time.time()
                        print("time: {} s\nnum_fish: {}\nThroughput: {} fish/min".format(end_time-start_time, len(check_type_fish), 60*len(check_type_fish)/(end_time-start_time)))
                        predict_fish = max(check_type_fish, key=check_type_fish.count)
                        print(predict_fish)
                        thr3 = Thread(target=call_arduino, args=[predict_fish, select_fishes])
                        thr3.start()
                        check_type_fish = []
                        no_change = 0
                        print("........................................................................\n")
...
if __name__ == '__main__':
    ...
    # selected_fishes = ["pod","ku_lare","see_kun",'too','khang_pan','hang_lueang','sai_dang', 'sai_dum']
    thr1 = Thread(target=detect_fish, args=["0", ["yolov7_4000_6.pt"], 
                False, True, 640, False, True, 'runs/detect', 'name', False,
                'cpu', False, 0.25, 0.45, None, False, True, ['sai_dum']])
