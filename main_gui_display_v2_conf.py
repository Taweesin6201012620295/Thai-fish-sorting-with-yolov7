...
from hd_nanpy4 import convenyor_run, convenyor_stop, call_arduino
                ...
                for i, det in enumerate(pred):  # detections per image
                    ...
                    if len(det): # !!!!!!!
                        ...
                        if len(check_type_fish) > 0:
                            no_change += 1
                            if no_change > 2:
                                count_type_fish = [check_type_fish.count("pod"),check_type_fish.count("ku_lare"),check_type_fish.count("too")
                                                   ,check_type_fish.count("see_kun"),check_type_fish.count("khang_pan"),check_type_fish.count("hang_lueang")
                                                   ,check_type_fish.count("sai_dang"),check_type_fish.count("sai_dum")]
                                #fish_out = max(check_type_fish, key=check_type_fish.count)
                                if count_type_fish.count(max(count_type_fish)) == 1:
                                    fish_out = name_fish[(count_type_fish).index(max(count_type_fish))]
                                else:
                                    fish_out = check_type_fish[conf_fish.index(max(conf))]
                                print(fish_out)
